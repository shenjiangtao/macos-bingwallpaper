...
const path ProgramDir = string(getenv("HOME")) + "/Pictures" + "/" + "BingWallpapers/"; // "/.local/" + ProgramName + "/";
...
int main(const int argc, char* argv[]) {
    ...
    cerr << "update " << ProgramDir << " " << ConfigName << endl;
...
void updateConfig(const path& configpath, const json& jsonData) {
    ...
    if (ofstream file(configpath); file.is_open()) {
        ...
        cerr << "Failed to open file " << configpath << endl;
    }
}
